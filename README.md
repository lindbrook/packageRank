...
functions, either set the argument for the filter you want to TRUE:
...
Ideally, we’d like to use both sets. However, the second set can be
computationally expensive. This is because when making relative
comparisons, like computing rank percentiles, we need to apply the
package specific filters to all the observed packages in a log. This can
mean tens of thousands of packages. While not unfeasible, currently this
will take a long time. For this reason, when setting
`all.filters = TRUE`, certain functions default to use only CRAN
specific filters.

The functions that use both CRAN and package specific functions when
`all.filters = TRUE` are `packageLog()`, `packageCountry()`, and
`filteredDownloads()`. The functions that use only CRAN specific
functions when `all.filters = TRUE` include `packageRank()`,
