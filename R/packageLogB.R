...
packageLogB <- function(packages = "cholera", date = NULL, all.filters = FALSE,
  ...
  out <- parallel::mclapply(packages, function(p) {
    ...
    if (small.filter) pkg.data <- smallFilterB(pkg.data)
    if (sequence.filter) pkg.data <- sequenceFilterB(pkg.data, p, ymd)  
    if (size.filter) pkg.data <- sizeFilterB(pkg.data, p)
