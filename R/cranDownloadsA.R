...
cranDownloadsA <- function(package = NULL, when = NULL, from = NULL,
  ...
  if (!is.null(package)) {
    if (!"R" %in% package) {
      ...
      first.published <- do.call(c, lapply(package, function(pkg) {
  ...
  if (is.null(when) & is.null(from) & is.null(to)) {
    ...
    if (!is.null(to)) {
      ...
      if (any(end.date.test)) {
        drop.pkgs <- paste(package[end.date.test], collapse = ", ")
        message("Note: ", drop.pkgs, " not published by selected end date.")
        package <- package[!end.date.test]
    ...
    if (is.null(package)) {
      ...
      to.data <- lapply(seq_along(package), function(i) {
        cranlogs::cran_downloads(package[i], from = first.published[i],
          to = end.date)
      })
      if ("R" %in% package) {
  ...
  if (!is.null(package)) {
    if (all(package != "R")) {
      if (check.package) {
        out$cranlogs.data <- packageLifeFilter(out, package, first.published) 
      }
    }
  }
