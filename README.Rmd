...
To query the log for a specific count, rank or rank percentile, use the functions below:

#### queryCount()

To find the packages that had 100 downloads (the default is 1, least observable number of downloads):
...
If you want the packages with a particular rank percentile, use `queryPercentile()`. Note that due to the discrete nature of counts, your choice of percentile may not be observed because they may fall in the vertical gaps in the data:

```{R, discrete_counts, echo = FALSE}
tmp <- cranDistribution(date = "2024-08-01")
plot(tmp$data$percentile, pch = 46, xlab = "Nominal Rank", ylab = "Rank Percentile", main = "Observed Rank Percentiles")
```
...
You can also set a range of rank percentiles using the 'lo' and/or 'hi' arguments. If you get an error message, you may need to widen your interval:
