...
The default unit of observation for `cranDownloads()` is the "day". Sometimes, however, viewing the data from a less granular perspectives can be illuminating. To do so, pass "month", or "year" to the function's `unit.observation` argument.

Consider the example below. In the first graph, I plot the daily downloads for [‘cranlogs'](https://CRAN.R-project.org/package=cranlogs) over its entire history (let's imagine today's date is "2022-04-15").
...
Here's the plot for the same data aggregated by month (with a smoother added):
...
Three things to notice about the aggregated plot. First, if an observation is still in-progress, that observation will be split in two (far right): 1) a point for the in-progress total (the black empty square) and 2) a point for the projected or estimated total (the red empty circle). The estimate is based on the proportion of the unit of observation completed. In the plot above, the 635 total downloads from April 1 through April 15 translates into an estimate of 1,270 downloads for the entire month (30 / 15 * 635). Second, all other points (if plotted) represent the total count at the end of an aggregate period. For example, the first observation (far left) records the total download count for ['cranlogs'](https://CRAN.R-project.org/package=cranlogs) for its first month on [CRAN](https://cran.r-project.org/) (May 2015). Not that each point is plotted on the x-axis on the final date of the aggregate period (on May 31 for the example above). Third, if you include a smoother, the curve only uses complete, not in-progress data.
