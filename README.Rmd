...
The default unit of observation for `cranDownloads()` is the "day". Sometimes, however, viewing the data from a less granular perspectives can be useful. To do so, you can pass "week", "month", or "year" to plot()'s `unit.observation` argument. 

##### `unit.observation = "month"`

The graph below plots the total daily downloads for [‘cranlogs'](https://CRAN.R-project.org/package=cranlogs) over its entire history (let's imagine today's date is "2022-04-15").
...
The next graph plots same data aggregated by month (with an added smoother):
...
There are three things to notice about the monthly plot. First, if the current month (far right) is still in-progress, that observation will be split in two: one point (the empty black square) for the in-progress total, the other  (the empty red circle) for the estimated total. The estimate is based on the proportion of the month completed. For example, in the plot above, the 635 downloads from April 1 through April 15 translates into an estimate of 1,270 downloads for the entire month (30 / 15 * 635). Second, all other points (if plotted) represent the total count at the end of a month. For example, the first observation (far left) records the total download count for ['cranlogs'](https://CRAN.R-project.org/package=cranlogs) for its first month on [CRAN](https://cran.r-project.org/) (May 2015). Note that each point is plotted on the x-axis on the final date of the aggregate period (May 31 in the example above). Third, if you include a smoother, the curve only uses complete, not in-progress data.

##### `unit.observation = "week"`

The graph below plots the daily downloads for 'cranlogs' from 01 January through 15 June 2022.
...
The next graph plots the same data aggregated by week (weeks begin on Sunday).
...
Because it's likely that both the start and end date will either be truncated or in-progress, you'll often see an inverted u-shape at the two endpoints of graph. To give a more representative picture of what's going on, I add two graphical annotations. The first, like the monthly plots, splits the current week if it's still in-progress (not Sunday) into an observed and an estimated point. The second, if needed, extends the first observation *back* to complete the first week (blue asterisk). Like the monthly plot, any added smoother will include only "complete" data points.
