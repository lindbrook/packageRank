...
Because packages with zero downloads are not recorded in the log, there is a potential censoring problem. However, my analysis indicates that the number of packages available on CRAN, but not in its archive, without any downloads on a given day is actually pretty small (more about this in future releases). In fact, the number of archived packages that are downloaded is often pretty high (e.g., upwards of 50%).
