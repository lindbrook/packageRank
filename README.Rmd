...
NOTE: ['packageRank'](https://CRAN.R-project.org/package=packageRank) relies on the ['cranlogs'](https://CRAN.R-project.org/package=cranlogs) package and requires an active internet connection. ['cranlogs'](https://CRAN.R-project.org/package=cranlogs) uses the [logs](http://cran-logs.rstudio.com/) from what was previously [RStudio's CRAN mirror](https://cran.rstudio.com) and is currently the "0-Cloud" mirror, "sponsored by RStudio", to compute package downloads. The logs for the previous day are generally posted at 18:00 (GMT+1) or 17:00 UTC (GMT+2) (daylight saving time); results for functions that rely on 'cranlogs' are available soon after.
