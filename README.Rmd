...
Details about the packge's functionality, how the package actually works, and and some potential issues of note are broken down in the sections below.

* [I Package Download Counts](#i---computing-package-download-counts) describes how `cranDownloads()` extends the functionality of [`cranlogs::cran_downloads()`](https://r-hub.github.io/cranlogs/) by adding a more user-friendly interface and a generic R `plot()` method that makes visualization easy.
* [II Package Download Rank Percentiles](#ii---computing-package-download-rank-percentiles) describes how `packageRank()` uses rank percentiles, a nonparametric statistic that computes the percentage of packages with fewer downloads, to give you your package's download count _relative_ to _all_ other [CRAN](https://CRAN.R-project.org/) packages.
* [III Package Download Filters](#iii---filtering-package-download-counts) describes the functions that filter out software and behavioral artifacts in the download logs that contribute to inflated download counts.
* [IV Availability of Logs and Results](#iv---availability-of-logs-and-results) answers the when and how questions about the availability of results. It also describes `logInfo()`, which is the easiest way to check the status of today's results.
* [V Data Fixes](#v---data-fixes-for-2012-and-2013) discusses two functions,`fixDate_2012()` and `fixCranlogs()`, which address and document data problems with logs from 2012 and 2013.
* [VI Et Cetera](#vi---et-cetera) discusses country code top-level domains, memoization, time zone effects, the internet connection time out problem, and recent (and ongoing) spikes in the downloads of Windows version of the R application on Sundays.
