...
To apply these filters in other ['packageRank'](https://CRAN.R-project.org/package=packageRank) functions, either set the argument for the filter you want to TRUE:
...
Ideally, we'd like to use both sets. However, the second set can be computationally expensive. This is because when making relative comparisons, like computing rank percentiles, we need to apply the package specific filters to all the observed packages in a log. This can mean tens of thousands of packages. While not unfeasible, currently this will take a long time. For this reason, when setting `all.filters = TRUE`, certain functions default to use only CRAN specific filters. 

The functions that use both CRAN and package specific functions when `all.filters = TRUE` are `packageLog()`, `packageCountry()`, and `filteredDownloads()`. The functions that use only CRAN specific functions when `all.filters = TRUE` include `packageRank()`, `ipPackage()`, `countryPackage()`, `countryDistribution()` and `packageDistribution()`.

### IV - notes

#### country codes (top level domains)

While IP addresses are anonymized, `packageCountry()` and `countryPackage()` make use of the fact that the logs attempt to provide corresponding ISO country codes or top level domains (e.g., AT, JP, US). Note however, that his covers about 85% of observations (i.e., approximately 15% country codes are NA). Also, for what it's worth, there seems to be a a couple of typos for country codes: "A1" (A + number one) and "A2" (A + number 2). According to [RStudio's documentation](http://cran-logs.rstudio.com/), this coding was done using MaxMind's free database, which no longer seems to be available.
