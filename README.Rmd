...
['packageRank'](https://CRAN.R-project.org/package=packageRank) is an R package that helps put package download counts into context. It does so via two core functions, `cranDownloads()` and `packageRank()`, a set of filters that reduces package download count inflation, and other assorted functions.
...
* [IV Availability of Logs and Results](#iv---availability-of-logs-and-results) answers the when and how question regarding the availability and unavailability of results, and discusses the utility of `logInfo()`.
* [V Data Fixes](#v---data-fixes-for-2012-and-2013) discusses two functions,`fixDate_2012()` and `fixCranlogs()`, that address data problems with logs from 2012 and 2013.
* [VI Et Cetera](#vi---et-cetera) discusses issues associated with country code top-level domains, memoization, time zones, the internet connection time out problem, and recent ongoing spikes in the Sunday downloads of Windows version of the R application.
...
Lowess is the default smoother. You can control the degree of smooth with the span argument (the default is span = 0.75):
...
### IV - availability of logs and results

To answer this question, you need to understand that [‘packageRank'](https://CRAN.R-project.org/package=packageRank) has two upstream, online dependencies. The first is Posit/RStudio's [CRAN package download logs](http://cran-logs.rstudio.com/), which record traffic to the “0-Cloud” mirror at cloud.r-project.org (formerly Posit/RStudio's CRAN mirror). The second is Gábor Csárdi's [‘cranlogs'](https://CRAN.R-project.org/package=cranlogs) R package, which using those logs to compute the download counts of both the R application and of R packages.

The [CRAN package download logs](http://cran-logs.rstudio.com/) for the _previous_ day are usually be posted by 17:00 UTC and the results for [‘cranlogs'](https://CRAN.R-project.org/package=cranlogs) are usually available soon after.

#### why aren't today's logs and results available?

Occasionally problems with "today's" data can emerge due to the upstream nature of the dependencies (illustrated below).

```
CRAN Download Logs --> 'cranlogs' --> 'packageRank'
```

If there's a problem with the [logs](http://cran-logs.rstudio.com/) (e.g., they're not posted on time), both [‘cranlogs'](https://CRAN.R-project.org/package=cranlogs) and [‘packageRank'](https://CRAN.R-project.org/package=packageRank) will be affected. If this happens, you'll see things like an unexpected zero count(s) for your package(s) (actually, it's a zero download count for all of [CRAN](https://cran.r-project.org/)), data from "yesterday", or a "Log is not (yet) on the server" error message.

```
'cranlogs' --> packageRank::cranDownloads()
```

If there's a problem with [‘cranlogs'](https://CRAN.R-project.org/package=cranlogs) but not with the [logs](http://cran-logs.rstudio.com/), only `packageRank::cranDownalods()` will be affected (the zero downloads problem). All the other [‘packageRank'](https://CRAN.R-project.org/package=packageRank) functions should work since they either directly access the logs or some other source. Usually, these errors resolve themselves the next time the underlying scripts are run ("tomorrow", if not sooner). 
...
### V - data fixes for 2012 and 2013
...
The Posit/RStudio download logs began on 01 October 2012. Each day's log is stored as a separate file with a name/URL that embeds the log's date:

```
http://cran-logs.rstudio.com/2022/2022-01-01.csv.gz
```

For the 2012 logs, this convention was broken: 1) some logs are duplicated (same log, multiple names), 2) at least one is mislabeled, and 3) the logs from 13 October through 28 December are offset by +3 days (e.g., the file with the name/URL "2012-12-01" contains the log for "2012-11-28"). You can read the details [here](https://github.com/lindbrook/packageRank/blob/master/docs/logs.md). Despite all this, only the last 3 logs of 2012 were lost.

Because ['packageRank'](https://CRAN.R-project.org/package=packageRank) functions like packageRank()` and `packageLog()` directly access logs via their filename/URL, e.g., `, I've written [fixDate_2012()](https://github.com/lindbrook/packageRank/blob/master/R/fixDate_2012.R) to re-map and resolve the issues above so that you'll get the log you expect.

#### `fixCranlogs()`

The situation for `packageRank::cranDownloads()` is different because that function is a wrapper of `cranlogs::cran_download()`. On the plus side, this means that it's not affected by the second and third problems. It's my understanding that this is because ['cranlogs'](https://CRAN.R-project.org/package=cranlogs) uses the date in the log rather than log's filename/URL. On the minus side, this means that functions and packages that depend on ['cranlogs'](https://CRAN.R-project.org/package=cranlogs) (e.g., ['adjustedcranlogs'](https://CRAN.R-project.org/package=adjustedcranlogs), ['dlstats'](https://CRAN.R-project.org/package=dlstats), [`packageRank::cranDownloads()`](https://github.com/lindbrook/packageRank/blob/master/R/cranDownloads.R)) are susceptible to the duplicate log problem.

Because ['cranlogs'](https://CRAN.R-project.org/package=cranlogs) relies on the date in the log rather than the date in the log's name/URL, it appears that ['cranlogs'](https://CRAN.R-project.org/package=cranlogs) can't detect multiple instances of logs with the same date. I found 3 logs with duplicate filename/URLs, and 5 additional instances of overcounting (including one of tripling).
...
The calendar date (e.g. "2021-01-01") is the unit of observation for ['packageRank'](https://CRAN.R-project.org/package=packageRank) functions. However, because you're typically interested in _today's_ log file, time zone differences may come into play.
...
However, depending on _where_ you make this request, you may not get the data you expect. In Honolulu, USA, you will. In Sydney, Australia you won't. The reason is that you've somehow forgotten a key piece of trivia: Posti/RStudio typically posts _yesterday's_ log around 17:00 UTC the following day.
