...
* [V Notes](#v---notes) discusses issues associated with country code top-level domains, memoization, time zone effects, and the internet connection time out problem.
...
Usually, these errors resolve themselves the next time the underlying scripts are run (typically "tomorrow", if not sooner). For more details, see the "time zones" section in [V Notes](#v---notes).
...
For those who may be interested in directly using the [RStudio download logs](http://cran-logs.rstudio.com/), this section describes some issues that may be of use.
...
I've patched packageRank::cranDownloads() with [fixCranlogs()](https://github.com/lindbrook/packageRank/blob/master/R/fixCranlogs.R) to fix this overcounting problem. The function recomputes the data using the actual logs when any of the eight problematic dates are requested. The details about the 8 days and `fixCranlogs()` can be found [here](https://github.com/lindbrook/packageRank/blob/logs/docs/logs.md).
