...
* [V Data Fix A](#v---data-fix-a) discusses issues stemming from problems with the actual logs from the end of 2012 through the beginning of 2013. These are fixed in `fixDate_2012()` and `fixCranlogs()`. 
* [VI Data Fix B](#vi---data-fix-b) discusses a "doubling" R application download counts stemming from problems with 'cranlogs' between 2023-09-13 through 2023-10-02. This is fixed in `fixRCranlogs()`.
...
The first data fix to note is more historical. It affects logs from 2012. It has two sources: one stems from problems with the logs when Posit/RStudio first began posting them; the other stems from how ['cranlogs'](https://CRAN.R-project.org/package=cranlogs) counts downloads. `fixDate_2012()` addresses the former. `fixCranlogs()` addresses the latter.

#### `fixDate_2012()` 

The Posit/RStudio download logs begin on 01 October 2012. Each day's log is a separate file with a name/URL that embeds that log's date:

```
http://cran-logs.rstudio.com/2022/2022-01-01.csv.gz
```

For the 2012 logs, this convention was broken in three ways: 1) some logs are duplicated (same log, multiple names), 2) at least one is mislabeled and 3) the logs from 13 October through 28 December are offset by +3 days (e.g., the file with the name/URL "2012-12-01" contains the log for "2012-11-28"). You can read the details [here](https://github.com/lindbrook/packageRank/blob/master/docs/logs.md). Despite all this, only the last 3 logs of 2012 were lost.

Because ['packageRank'](https://CRAN.R-project.org/package=packageRank) functions like `packageRank()` and `packageLog()` work by directly accessing logs via their filename/URL, they, unlike 'cranlogs'](https://CRAN.R-project.org/package=cranlogs) functions, are affected by these problems. [fixDate_2012()](https://github.com/lindbrook/packageRank/blob/master/R/fixDate_2012.R) re-maps the problematic logs so that you'll get the logs you expect.

#### `fixCranlogs()`

The situation for `packageRank::cranDownloads()` is different. This is because that function is a wrapper for `cranlogs::cran_download()`. On the plus side, it's not affected by the second and third log problems. My understanding is that this is because ['cranlogs'](https://CRAN.R-project.org/package=cranlogs) uses the date in the log rather than a log's filename/URL. On the minus side, this means that functions and packages that depend on ['cranlogs'](https://CRAN.R-project.org/package=cranlogs) (e.g., ['adjustedcranlogs'](https://CRAN.R-project.org/package=adjustedcranlogs), ['dlstats'](https://CRAN.R-project.org/package=dlstats), [`packageRank::cranDownloads()`](https://github.com/lindbrook/packageRank/blob/master/R/cranDownloads.R)) are susceptible to the first log problem (duplicate names). Because ['cranlogs'](https://CRAN.R-project.org/package=cranlogs) relies on the date in the log rather than the date in the log's name/URL, it appears that ['cranlogs'](https://CRAN.R-project.org/package=cranlogs) can't detect multiple instances of logs with the same date. I found 3 logs with duplicate filename/URLs, and 5 additional instances of overcounting (including one of tripling).

[fixCranlogs()](https://github.com/lindbrook/packageRank/blob/master/R/fixCranlogs.R) addresses this overcounting problem by recomputing the data using the actual logs when any of the eight problematic dates are requested. 
Details about the 8 days and `fixCranlogs()` can be found [here](https://github.com/lindbrook/packageRank/blob/master/docs/logs.md).

### VI - data fix B

The second data fix to note is of more recent vintage. From 2023-09-13 through 2023-10-02, the download counts for the R application returned by `cranlogs::cran_downloads(packages = "R")`, is, with two exceptions, twice what one would expect when looking at the actual log(s). The two exceptions are: 1) on 2023-09-28 the counts are identical but for a "rounding error" possibly due to an NA value and 2) on 2023-09-30 there is actually a three-fold difference. 
...
Details and code for replication can be found in issue [#69](https://github.com/r-hub/cranlogs/issues/69). Assuming the logs are right, `fixRCranlogs()` corrects the discrepancies.

Note that there was a similar issue for package download counts around the same period but it is now fixed in ['cranlogs'](https://CRAN.R-project.org/package=cranlogs). For details, see issue [#68](https://github.com/r-hub/cranlogs/issues/68)
...
The graph above for [R downloads](#packages--r) shows the daily downloads of the R application broken down by platform (Mac, Source, Windows). In it, you can see the typical weekly pattern of mid-week peaks and weekend troughs. 
