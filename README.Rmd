...
* [I Download Counts](#i---download-counts) describes how `cranDownloads()` gives [`cranlogs::cran_downloads()`](https://r-hub.github.io/cranlogs/reference/cran_downloads.html) a more user-friendly interface, and how it makes visualizing those data easy (via a generic R `plot()` method).
...
Note that since Sunday 06 November 2022 and Wedenesday, 18 January 2023, there've been spikes of downloads of the Windows version of R on Sundays and Wednesdays (details below in [R Windows Sunday and Wednesday downloads](#r-windows-sunday-and-wednesday-downloads)).
...
The graph above for [R downloads](#packages--r) shows the daily downloads of the R application broken down by platform (Mac, Source, Windows). In it, you can see the typical weekly pattern of mid-week peaks and weekend troughs. However, starting Sunday, 06 November 2022 and Wednesday, 18 January 2023, this pattern was broken. On subsequent Sundays and Wednesdays, there are noticeable spikes (almost an order of magnitude difference) in the downloads of the Windows version of R. 
...
This pattern does not extend to the Mac or Source versions. You can see this in the graphs below. In each plot, the the watershed date is plotted as a vertical line (red for Sunday; blue for Wednesday). Sunday and Wednesdays before the watershed are plotted with empty circles. If Sunday and Wednesdays after their respective watershed appear to be outliers, they are plotted with filled circles; otherwise, they are plotted with empty circles.

```{r sunday_platform, echo = FALSE}
dat <- cranDownloads("R", from = "2022-10-15", to = "2023-02-22")$cranlogs.data
sundays <- seq.Date(from = as.Date("2022-10-16"), to = as.Date("2023-02-22"), 
  by = 7)
wednesdays <- seq.Date(from = as.Date("2022-10-19"), 
  to = as.Date("2023-02-22"), by = 7)
...
ante.sun <- sundays < sun
post.sun <- sundays >= sun

wed <- as.Date("2023-01-18")
ante.wed <- wednesdays < wed
post.wed <- wednesdays >= wed
```
...
points(osx[osx$date %in% sundays[ante.sun], c("date", "count")], pch = 1, 
  col = "red")
points(osx[osx$date %in% sundays[post.sun], c("date", "count")], pch = 1, 
  col = "red")
points(osx[osx$date == as.Date("2022-11-20"), c("date", "count")], pch = 16, 
  col = "red")
text(osx[osx$date == as.Date("2022-11-20"), c("date", "count")], pos = 4, 
  labels = "Nov 20", cex = 0.75, col = "red")
title(main = "R Mac Sundays")

plot(osx$date, osx$count, type = "l", xlab = "Date", ylab = "Count")
axis(3, at = wed, labels = "2023-01-18", cex.axis = 2/3, padj = 0.9, 
  col.axis = "blue", col.ticks = "blue")
abline(v = wed, lty = "dotted", col = "blue")
points(osx[osx$date %in% wednesdays[ante.wed], c("date", "count")], pch = 1, 
  col = "blue")
points(osx[osx$date %in% wednesdays[post.wed], c("date", "count")], pch = 1, 
  col = "blue")
...
axis(3, at = sun, labels = "2022-11-06", cex.axis = 2/3, padj = 0.9, 
  col.axis = "red", col.ticks = "red")
abline(v = sun, lty = "dotted", col = "red")
points(src[src$date %in% sundays[ante.sun], c("date", "count")], pch = 1, 
  col = "red")
points(src[src$date %in% sundays[post.sun], c("date", "count")], pch = 1, 
  col = "red")
points(src[src$date == as.Date("2022-11-20"), c("date", "count")], pch = 16, 
  col = "red")
text(src[src$date == as.Date("2022-11-20"), c("date", "count")], pos = 4, 
  labels = "Nov 20", cex = 0.75, col = "red")
...
axis(3, at = wed, labels = "2023-01-18", cex.axis = 2/3, padj = 0.9, 
  col.axis = "blue", col.ticks = "blue")
abline(v = wed, lty = "dotted", col = "blue")
points(src[src$date %in% wednesdays[ante.wed], c("date", "count")], pch = 1, 
  col = "blue")
points(src[src$date %in% wednesdays[post.wed], c("date", "count")], pch = 1, 
  col = "blue")
...
axis(3, at = sun, labels = "2022-11-06", cex.axis = 2/3, padj = 0.9, 
  col.axis = "red", col.ticks = "red")
abline(v = sun, lty = "dotted", col = "red")
points(win[win$date %in% sundays[ante.sun], c("date", "count")], pch = 1, 
  col = "red")
points(win[win$date %in% sundays[post.sun], c("date", "count")], pch = 16, 
  col = "red")
...
axis(3, at = wed, labels = "2023-01-18", cex.axis = 2/3, padj = 0.9, 
  col.axis = "blue", col.ticks = "blue")
abline(v = wed, lty = "dotted", col = "blue")
points(win[win$date %in% wednesdays[ante.wed], c("date", "count")], pch = 1, 
  col = "blue")
points(win[win$date %in% wednesdays[post.wed], c("date", "count")], pch = 16, 
  col = "blue")
