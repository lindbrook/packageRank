...
`cranlogs::cran_download(packages = NULL)` computes the total number of package downloads from CRAN. You can visualize this by:
...
`cranlogs::cran_download(packages = "R")` computes the total number of downloads of the R application (note that you can only use "R" or a vector of packages names, not both!). You can visualize this by:
...
To visualize a package's downloads relative to that of "all" other packages over time:
