...
Note that since Sunday, 06 November 2022, and each subsequent Sunday, there've been spikes of downloads of the Windows version of R. Details below in [R Windows Sunday downloads](#r-windows-sunday-downloads).
...
The unit of observation for both `cranDownloads()` and `cranlogs::cran_dowanlods()` is the "day". The graph below plots the daily downloads for [‘cranlogs'](https://CRAN.R-project.org/package=cranlogs) from 01 January 2022 through 15 April 2022.
...
For the logs of 2012, this convention was broken: 1) some logs are duplicated (same log, multiple names), 2) at least one is mislabeled, and 3) the logs from 13 October through 28 December are offset by +3 days (e.g., the file with the name/URL "2012-12-01" contains the log for "2012-11-28"). You can read the details [here](https://github.com/lindbrook/packageRank/blob/master/docs/logs.md). Despite all this, only the last 3 logs of 2012 were lost.
...
Second, to help you check/remember when logs are posted in your location, there's `logInfo()`. The function checks to see if "today's" log is posted on RStudio's server and if "today's" results have been computed in 'cranlogs'. If you use the optional `logInfo(show.available = TRUE)`, the function will check the previous week to look up the latest available download log and 'cranlogs' result.
...
The graph above for [R downloads](#packages--r) plots the daily downloads of the R application broken down by platform (Mac, Source, Windows). In it, you can seek a weekly pattern with mid-week peaks and weekend dips. However, on Sunday, 06 November 2022 this pattern was broken. 

On that day, and each subsequent Sunday, there is a noticeable spike in downloads of the Windows version of the R application. 
...
This pattern does not extend to the Mac or Source versions of R (Sundays are plotted with filled circles):

```{r sunday_platform, echo = FALSE}
r.data <- cranDownloads("R", from = "2022-10-15", to = "2023-01-17")$cranlogs.data

sundays <- seq.Date(from = as.Date("2022-10-16"), to = as.Date("2023-01-15"), 
  by = 7)

win <- r.data[r.data$platform == "win", ]
osx <- r.data[r.data$platform == "osx", ]
src <- r.data[r.data$platform == "src", ]

sun <- as.Date("2022-11-06")
ante <- sundays < sun
post <- sundays >= sun
```

```{r sundays_mac, echo = FALSE}
plot(osx$date, osx$count, type = "o", xlab = "Date", ylab = "Count")
axis(3, at = sun, labels = "2022-11-06", cex.axis = 2/3, padj = 0.9)
abline(v = sun, col = "gray", lty = "dotted")
points(osx[osx$date %in% sundays, c("date", "count")], pch = 16)
title(main = "R Mac")
```

```{r sundays_src, echo = FALSE}
plot(src$date, src$count, type = "o", xlab = "Date", ylab = "Count")
axis(3, at = sun, labels = "2022-11-06", cex.axis = 2/3, padj = 0.9)
abline(v = sun, col = "gray", lty = "dotted")
points(src[src$date %in% sundays, c("date", "count")], pch = 16)
title(main = "R Source")
```

```{r sundays_win, echo = FALSE}
plot(win$date, win$count, type = "o", xlab = "Date", ylab = "Count")
axis(3, at = sun, labels = "2022-11-06", cex.axis = 2/3, padj = 0.8, 
  col.axis = "red", col.ticks = "red")
abline(v = sun, lty = "dotted", col = "red")
points(win[win$date %in% sundays[ante], c("date", "count")], pch = 16)
points(win[win$date %in% sundays[post], c("date", "count")], pch = 16, 
  col = "red")
title(main = "R Windows")
```
