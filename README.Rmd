...
* [V Data Fix A](#v---data-fix-a) discusses two problems with download counts. The first stems from problems with the logs from the end of 2012 through the beginning of 2013. These are fixed in `fixDate_2012()` and `fixCranlogs()`. 
...
You can control the degree of smooth using `f` argument for 'base' graphics  (the default is f = 2/3) and the `span` argument with 'ggplot2' graphics (the default is span = 0.75):
