...
The '[cranlogs](https://cran.r-project.org/package=cranlogs)' package computes the number of downloads, be it packages or the R appliction itself, from RStudio's [CRAN mirror](http://cran-logs.rstudio.com).
...
For Bioconductor packages, use `bioconductorDownloads()`. Note that logs are aggreated to the month or the calendar year, so dates must either be "yyyy-mm" or "yyyy".
